...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['registrar'])) {
        ...
        if (empty($nombre)) {
            ...
            try {
                ...
                $stmt = $conn->prepare("SELECT id_cliente, telefono, cedula FROM clientes WHERE telefono = ? OR cedula = ?");
                ...
                if ($resultado->num_rows > 0) {
                    ...
                    if ($cliente['telefono'] === $telefono && $cliente['cedula'] === $cedula) {
                        $error = "Este número y cédula ya están registrados. <a href='login_cliente.php' style='color:#FFAA00;'>¿Iniciar sesión?</a>";
                    } elseif ($cliente['telefono'] === $telefono) {
                        $error = "Este número de teléfono ya está registrado";
                    } else {
                        $error = "Este número de cédula ya está registrado";
                    }
                } else {
                    // Generar código de verificación
                    $codigo_verificacion = str_pad(rand(0, 999999), 6, '0', STR_PAD_LEFT);
                    
                    // Insertar nuevo cliente con transacción
                    ...
                    try {
                        ...
                        if ($stmt->execute()) {
                            ...
                            // En producción, aquí iría el envío real del SMS
                            // enviarSMS($telefono, "Tu código de verificación es: $codigo_verificacion");
                            
                            $conn->commit();
                        } else {
                            throw new Exception("Error al ejecutar la inserción: " . $conn->error);
                        }
                    } catch (Exception $e) {
                        $conn->rollback();
                        throw $e;
                ...
                $error = "Error en el sistema. Por favor intenta nuevamente más tarde.";
    ...
    if (isset($_POST['verificar'])) {
        ...
        if (empty($codigo_ingresado)) {
            ...
            $show_verification = true;
        } elseif ($codigo_ingresado === $codigo_correcto) {
            // Actualizar estado de verificación con transacción
            $conn->begin_transaction();
            
            try {
                $stmt = $conn->prepare("UPDATE clientes SET verificado = 1, estado = 'activo', codigo_verificacion = NULL WHERE telefono = ?");
                $stmt->bind_param("s", $telefono);
                $stmt->execute();
                
                // Establecer sesión
                $_SESSION['id_cliente'] = $stmt->insert_id;
                $_SESSION['nombre'] = $_SESSION['nombre_registro'];
